...
| 5 | グラフ分析の強化 + 資産推移グラフ | 完了 |
| 6 | DBバックアップ（ダウンロードのみ） | 完了 |
...
## 単位5: グラフ分析の強化 + 資産推移グラフ（完了）

Codexの独立分析で、当初案の仕様不整合（単月関数`budgetActuals`を「選択月」の概念がない年間画面に配置する想定になっていた）が見つかり、設計を修正した。`budgetActuals`は月次画面（`MonthlySummary.svelte`）に単位3で実装済みだったインライン計算をテスト可能な純粋関数へ抽出するリファクタリングとして扱い、年間画面には配置しないことにした。年間画面（`AnnualSummary.svelte`）へは「カテゴリ別月次推移」（積み上げ棒、新規`CategoryMonthlyChart.svelte`）と「資産（支払方法別）残高推移」（新規`PaymentMethodBalanceChart.svelte`）の2セクションのみ追加し、肥大化を避けるためサブコンポーネントに分割した。

`paymentMethodBalanceTrend`は、選択年より前の取引も初期残高に繰り込んで累積する設計にした（年内データだけで計算すると、初期残高設定より前から取引がある支払方法で残高が不正確になるため）。残高は負値になり得るため、資産推移グラフはゼロ基準線から上下に伸びる発散型バーで表現する。

Codexレビューで1件のブロッキング指摘（発散型バーの基準線とバー自体が異なるCSS座標空間を基準にしていたため、視覚的なゼロ線とバーの分岐点がずれていた）があり対応済み。`mise run web-ci`グリーン確認済み（40テスト成功）。コミット `10d9c24`。

なお、このレビュー中にセッションが再起動し、進行中だったCodexレビュージョブが11時間以上ハングする事象が発生した（進捗ログが完全に停止していたため、キャンセルして新しいスレッドで再依頼した）。原因はセッション再起動によるものと推測されるが、根本原因の特定はしていない。

## 単位6: DBバックアップ（ダウンロードのみ）（完了）

`GET /api/backup` を追加し、接続済みSQLiteプールから `VACUUM INTO` で一意な一時ファイルへ整合したバックアップを作成する。ファイルはメモリへ読み込んだ後、成功・失敗を問わず削除を試みる。ダウンロード名の日付はSQLiteのローカル時刻を基準とし、レスポンスにはバイナリ形式、添付ファイル名、バイト長、キャッシュ禁止の各ヘッダを設定する。

設定の「データ管理」画面にはプレーンリンクの「フルバックアップ」カードを追加した。READMEには、既存のDocker Compose named volume `kakeibo-data` を対象とする手動リストア手順と、`-wal` / `-shm` sidecarファイル確認の注意事項を追記した。アプリ内のリストア／アップロード機能は対象外のままとする。
