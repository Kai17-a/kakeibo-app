...
現状の仕様を棚卸しし、不足/追加候補として挙がった7機能＋追加で見つかった4機能（計8機能・6実装単位）を実装するロードマップの進捗を記録する。全機能実装完了。

対象外（実装しない）: レシート画像添付、定期支出の支払日リマインド、認証・マルチユーザー対応。

## スプレッドシート版との機能差分解消（完了）

月次・年間サマリーは、有効期間内かつ未計上の固定定期支出・定期収入を予測として合算する。既に同じ定期設定から明細が生成されている月は予測を加算せず、実績との二重計上を避ける。年間画面には全支出カテゴリの1〜12月と年間合計を確認できる数値表を追加した。

定期支出の編集時には、今月以降に生成済みの明細へ金額・カテゴリ・支払方法・備考を同期するか選択できる。過去月は家計簿の確定実績として変更しない。

定期支出には任意の外貨金額、3文字通貨コード、円換算レートを保存できる。画面では3項目から円換算額を算出して通常の金額へ登録する。外部為替APIとの連携とレート履歴は対象外。
...
ロードマップの機能を依存関係に基づく実装単位へ再編し、すべて完了した。各単位の設計判断・実装内容・レビュー指摘は本ファイルの各セクションを参照。

| 単位 | 内容 | 状態 |
| --- | --- | --- |
| 1 | 資産・口座残高管理（`payment_methods.initial_balance`、`incomes.payment_method_id`） | 完了 |
| 2 | 収入明細一覧UI・収入明細フィルタ | 完了 |
| 3 | 予算管理（`budgets`テーブル） | 完了 |
| 5 | グラフ分析の強化 + 資産推移グラフ | 完了 |
| 6 | DBバックアップ（ダウンロードのみ） | 完了 |

単位4（Webhookイベント種別フィルタ）は、その後Webhook機能自体を廃止したため削除した（migration `0012__drop_webhook_urls`）。

対象外（実装しない）: DB復元のアプリ内実装（`SqlitePool`が`max_connections(1)`で全ルーターに直接配線されており、稼働中の安全な差し替え手段がないため）、目標金額を登録する貯蓄目標機能（口座残高の推移グラフのみ実装）。
...
`budgets`テーブル（`category_id` UNIQUE, `amount`）を追加し、カテゴリごとの恒久的な月額予算を管理する。カテゴリ削除保護を拡張し、`budgets`から参照されているカテゴリの削除もブロックする。
