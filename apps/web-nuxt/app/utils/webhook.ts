...
export function validateWebhook(state: Partial<WebhookUrlInput>) {
  ...
  try {
    ...
    errors.push({ name: 'url', message: 'http:// または https:// で始まる有効なURLを入力してください。' })
  }
  if (!state.events?.length) errors.push({ name: 'events', message: '1つ以上のイベントを選択してください。' })
  if ((state.description?.length ?? 0) > 500) errors.push({ name: 'description', message: '説明は500文字以内で入力してください。' })
  return errors
}
