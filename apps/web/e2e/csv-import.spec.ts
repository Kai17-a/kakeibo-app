  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/import/expenses/preview' && request.method() === 'POST') {
      await route.fulfill({
        ...
        body: JSON.stringify({
          rows: [
            { transaction_date: '2026-01-15', amount: '1200', category: '食費', category_is_new: true, payment_method: '現金', payment_method_is_new: true, description: '昼食' }
          ],
    ...
    if (path === '/api/import/expenses' && request.method() === 'POST') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ imported: 1, created_categories: ['食費'], created_payment_methods: ['現金'] })
      })
      return
    }
    if (path === '/api/import/incomes/preview' && request.method() === 'POST') {
      await route.fulfill({
        ...
        body: JSON.stringify({
          rows: [
            { transaction_date: '2026-01-15', amount: '300000', category: '給与', category_is_new: true, description: '1月分' }
          ],
    ...
    if (path === '/api/import/incomes' && request.method() === 'POST') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ imported: 1, created_categories: ['給与'], created_payment_methods: [] })
      })
      return
    }
    if (path === '/api/import/recurring-expenses/preview' && request.method() === 'POST') {
      await route.fulfill({
        ...
        body: JSON.stringify({
          rows: [
            { name: '家賃', amount: '61100', currency: '', foreign_amount: '', payment_day: '1', start_date: '2026-01-01', end_date: null, category: '住居費', category_is_new: true, payment_method: '口座振替', payment_method_is_new: true, is_variable: '', description: null }
          ],
    ...
    if (path === '/api/import/recurring-expenses' && request.method() === 'POST') {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ imported: 1, created_categories: ['住居費'], created_payment_methods: ['口座振替'] })
      })
      return
    }
...
test('CSVをプレビューしてから支出データをインポートする', async ({ page }) => {
  ...
  await expect(page.locator('[data-slot="title"]').filter({ hasText: '1件の支出をインポートしました' })).toBeVisible()
})
...
test('CSVをプレビューしてから収入データをインポートする', async ({ page }) => {
  ...
  await expect(page.getByText('1件をインポートします')).toBeVisible()
  await page.getByRole('button', { name: '登録する' }).click()
  await expect(page.locator('[data-slot="title"]').filter({ hasText: '1件の収入をインポートしました' })).toBeVisible()
})
...
test('CSVをプレビューしてから固定費データをインポートする', async ({ page }) => {
  ...
  await fileChooser.setFiles({
    name: 'recurring.csv',
    mimeType: 'text/csv',
    buffer: Buffer.from('名称,金額,通貨,外貨金額,支払日,開始日,終了日,カテゴリ,支払方法,金額変動,備考\n家賃,61100,,,1,2026-01-01,,住居費,口座振替,,\n')
  })
