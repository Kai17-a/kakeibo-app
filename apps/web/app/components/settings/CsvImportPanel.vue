...
import type { ImportKind, ImportResult, ExpensePreviewRow, IncomePreviewRow, RecurringExpensePreviewRow } from '~/types/import'

const props = defineProps<{ kind: ImportKind }>()

const label = computed(() => props.kind === 'expense' ? '支出' : props.kind === 'income' ? '収入' : '固定費')
...
async function onFileChange(event: Event) {
  ...
  try {
    ...
    const result = props.kind === 'expense'
    ...
    previewCreatedPaymentMethods.value = 'created_payment_methods' in result ? result.created_payment_methods : []
        ...
        >
          ...
          <template #category-cell="{ row }">
            <span>
              ...
              <UBadge
                v-if="row.original.category_is_new"
                color="secondary"
                variant="soft"
              >
                新規
