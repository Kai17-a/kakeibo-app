...
export function validateRecurringIncome(state: RecurringIncomeForm, categories: { id: string }[]) {
  ...
  if (!/^\d+$/.test(state.amount.trim()) || !Number.isSafeInteger(Number(state.amount)) || Number(state.amount) < 1) {
    errors.push({ name: 'amount', message: '金額を1以上の整数で入力してください。' })
  }
  const day = Number(state.payment_day)
  if (!Number.isInteger(day) || day < 1 || day > 31) errors.push({ name: 'payment_day', message: '入金日を1〜31の整数で入力してください。' })
  if (!categories.some(item => item.id === state.category_id)) errors.push({ name: 'category_id', message: '収入カテゴリを選択してください。' })
