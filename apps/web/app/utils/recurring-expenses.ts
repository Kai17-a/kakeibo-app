...
export function validateRecurringExpense(
  ...
  if (state.usdBased) {
    if (!Number.isFinite(Number(state.foreignAmount)) || Number(state.foreignAmount) <= 0) {
      errors.push({
        name: "foreignAmount",
        message: "毎月のUSD金額を0より大きい数値で入力してください。",
      });
    }
  } else if (
  ...
  if (!Number.isInteger(day) || day < 1 || day > 31)
    errors.push({ name: "payment_day", message: "支払日を1〜31の整数で入力してください。" });
  if (!categories.some((item) => item.id === state.category_id))
    errors.push({ name: "category_id", message: "支出カテゴリを選択してください。" });
  if (!paymentMethods.some((item) => item.id === state.payment_method_id))
    errors.push({ name: "payment_method_id", message: "支払方法を選択してください。" });
