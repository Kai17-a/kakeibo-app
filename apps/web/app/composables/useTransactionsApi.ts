...
export function useTransactionsApi() {
  ...
  return {
    ...
    createExpense: (body: ExpenseInput) =>
      client<Expense>("/api/expenses", { method: "POST", body }),
    updateExpense: (id: string, body: ExpenseInput) =>
      client<Expense>(`/api/expenses/${encodeURIComponent(id)}`, { method: "PUT", body }),
    ...
    updateIncome: (id: string, body: IncomeInput) =>
    ...
    previewRecurringExpenseExchangeRate: (id: string, month: string) =>
      client<ExchangeRatePreview>(
        `/api/recurring-expenses/${encodeURIComponent(id)}/exchange-rate`,
        { query: { month } },
      ),
  };
}
