...
async function save() {
  ...
  try {
    ...
    toast.add({
      title: editingId.value ? "Webhook URLを更新しました" : "Webhook URLを追加しました",
      color: "success",
    });
  } catch (error) {
...
<template>
  <section class="space-y-6" aria-labelledby="webhook-heading">
    ...
    <div v-if="loading" role="status" aria-label="Webhook URLを読み込み中" class="space-y-3">
    ...
    <UCard v-else-if="!items.length" class="text-center">
      <div class="space-y-3 py-10">
        <UIcon name="i-lucide-webhook" class="size-8 text-muted" />
        <h2 class="font-semibold">Webhook URLがありません</h2>
        <p class="text-sm text-muted">
    ...
    >
      ...
      <template #footer>
        ...
        <UButton type="submit" form="webhook-form" :loading="saving" :disabled="saving">
          {{ editingId ? "更新" : "追加" }}
        </UButton>
      </template>
    </UModal>
