  ...
  <section class="space-y-6" aria-labelledby="payment-method-heading">
    <div class="flex min-w-0 items-start justify-between gap-3 sm:gap-4">
      <div class="min-w-0 space-y-1">
        ...
        <p class="text-sm text-muted">支出の登録時に選択する支払方法と残高を管理します。</p>
      </div>
      <UButton
        icon="i-lucide-plus"
        class="shrink-0"
    ...
    <UCard v-else-if="!items.length" class="text-center">
      <div class="space-y-3 py-10">
        <UIcon name="i-lucide-wallet" class="size-8 text-muted" />
    ...
    >
      ...
      <template #footer>
        ...
        <UButton type="submit" form="payment-method-form" :loading="saving" :disabled="saving">
          {{ editingId ? "更新" : "追加" }}
        </UButton>
      </template>
    </UModal>
