...
watch(
  ...
  async () => {
    ...
    const monthList = Array.from(
      { length: 12 },
      (_, index) => `${year.value}-${String(index + 1).padStart(2, "0")}`,
    );
...
const projectedMonths = computed(() =>
  months.value.map((item) => {
    ...
    const forecast = recurringForecast(
      ...
      month,
      usdPreviewsByMonth.value.get(month),
    );
    return {
      ...item,
...
const chartMax = computed(() =>
  Math.max(1, ...projectedMonths.value.flatMap((item) => [item.income, item.expense])),
);
...
const categoryChartMax = computed(() =>
  Math.max(1, ...categoryMonths.value.map((item) => item.total)),
);
...
const maxPositive = computed(() =>
  Math.max(0, ...balanceMonths.value.flatMap((item) => item.values.map((value) => value.balance))),
);
            ...
            <UCard class="min-w-0">
              ...
              <div class="mt-4 flex gap-4 text-xs text-muted">
                <span class="flex items-center gap-1.5"
                  ><span class="size-2.5 bg-primary" />収入</span
                >
                <span class="flex items-center gap-1.5"
                  ><span class="size-2.5 bg-error" />支出</span
                >
              </div>
            </UCard>
            <UCard class="min-w-0">
                  ...
                  <div class="flex justify-between gap-3">
                    ...
                    <span class="tabular-nums">
                      ...
                      <small class="text-muted"
                        >{{
                          expenseTotal ? Math.round((item.total / expenseTotal) * 100) : 0
                        }}%</small
                      >
                    </span>
                  </div>
                  ...
                  >
                    <div class="flex h-[calc(100%_-_2rem)] flex-col-reverse justify-start">
                      <div
                        ...
                        class="w-full"
                        :style="{
                          height: `${(value.total / categoryChartMax) * 100}%`,
                    ...
                    <div class="relative h-[calc(100%_-_2rem)]">
                      ...
                      <div
                        ...
                        :style="{
                          ...
                          top:
                            value.balance >= 0
                              ? `${positiveHeight - (value.balance / balanceChartRange) * 100}%`
                              : `${positiveHeight}%`,
          ...
          <UCard>
            ...
            <div class="overflow-x-auto">
              <table class="w-full min-w-[900px] text-sm">
                ...
                <tbody>
                  <tr v-for="row in categoryRows" :key="row.id" class="border-t border-default">
