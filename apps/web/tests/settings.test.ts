...
import { groupCategories, categoryMove, availableBudgetCategories, validateNamed, validAmount } from '../app/utils/settings.ts'
...
test('groups parents and children by display order without mutating input', () => {
  assert.deepEqual(groupCategories(categories).map(c => c.id), ['a', 'a1', 'a2', 'b'])
...
test('moves only siblings, sends every sibling and keeps child relationships', () => {
  ...
  assert.deepEqual(groupCategories(moved.items).map(c => c.id), ['a', 'a2', 'a1', 'b'])
...
test('budget choices exclude assigned categories but retain the edited category', () => {
  ...
  assert.deepEqual(availableBudgetCategories(categories, budgets, 'a').map(c => c.id), ['a'])
