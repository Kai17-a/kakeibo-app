...
test("settings API uses the correct endpoints, queries and mutation payloads without retries", async () => {
  ...
  try {
    ...
    assert.deepEqual(calls.at(-1), {
      ...
      options: { query: { sort_by: "name", sort_order: "asc", per_page: 100 } },
    });
    ...
    assert.deepEqual(calls.at(-1), {
      ...
      options: { method: "POST", body: recurringInput },
    });
    await api.recurringIncomes.update("a/b", recurringInput);
    assert.deepEqual(calls.at(-1), {
      path: "/api/recurring-incomes/a%2Fb",
      options: { method: "PUT", body: recurringInput },
    });
    ...
    for (const [resource, path, input] of [
      [
        api.paymentMethods,
        "/api/payment-methods",
        { name: "現金", description: null, initial_balance: "0" },
      ],
